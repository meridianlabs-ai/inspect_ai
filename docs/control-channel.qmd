...
So that a forgotten `release` doesn't leak the process forever, a parked process auto-releases after 24 hours with no control-channel activity. Any request to the process — a read like `inspect ctl task list` or a mutation — resets the clock, so a park anyone is actually using never times out; the timer runs only while parked (never during an eval). On expiry the process releases exactly as `inspect ctl process release` would (clean exit, discovery removed) and says why on stderr; `--json` / `--detach` consumers see `"released": "idle_timeout"` on the final `done` record.

Tune the window at launch with a duration suffix, or opt out entirely:

```bash
inspect eval ctf.py --ctl-server=keep:4h       # 4 hours idle
inspect eval ctf.py --ctl-server=keep:forever  # park indefinitely
```

Durations accept `s` / `m` / `h` / `d` suffixes or bare seconds. On a running process, view or retune the window with `inspect ctl config --park-timeout <seconds>` (pass `clear` to restore the launch value) — setting it counts as activity, so the new window starts immediately. While parked, `inspect ctl task list` and `inspect ctl process list` show the auto-release countdown.
