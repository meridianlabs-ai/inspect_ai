...
| `inspect ctl task add` | Add a task to a running (`--ctl-server=keep`) eval process. |
...
`inspect ctl task add` submits a new task to a running eval process, so a warm process — models and connection pools warmed, sandboxes provisioned within a session, a keep-alive park held open — can take more work without paying a whole new `inspect eval` launch. `SPEC` uses exactly the `inspect eval` spelling (a registry name, or a file path with an optional `@task` selector) and resolves *inside the target process*: a registry name must be importable there, and file paths are sent absolute. A bare name is always tried as a registry name first (matching `inspect eval`, even when a same-named file or directory exists where you run the command) — spell a directory `./mytasks` or `mytasks/` to have it resolved as a path. Resolution runs synchronously on the target's event loop, so an add whose task construction is slow (a large dataset download, say) briefly stalls in-flight samples — whose wall-clock and working limits keep ticking meanwhile; this applies to `--dry-run` too. `-T` task args and `--model` work as they do on `inspect eval` (without `--model`, the task runs against every launch model — one added task per model).

```bash
$ inspect ctl task add evals/arc.py@arc_easy -T difficulty=hard
```

The added task runs under the same run id with its own log file in the run's log directory, and appears in `inspect ctl task list` / `sample list` like any sibling. If the eval is still running, the task starts as soon as there is free capacity (honoring `max_tasks`); if the process has finished and is parked under keep-alive, the park wakes, runs the added task, and re-parks when it drains. An add accepted during the handoff between those two states is buffered and never lost, and a buffered add runs even if a `release` arrives before it started.

Only an *addable* process accepts: one whose keep-alive intent is on — launched with `--ctl-server=keep`, or upgraded mid-run with `inspect ctl process keep` (without the park there would be a window where an accepted add could be lost to teardown, and accepted must mean "will run"). Other processes reject with an error naming both fixes; eval-set processes are not yet supported. `--dry-run` resolves and validates the spec — reporting the same per-task rows (task id, model, sample count, epochs, both honoring the run's launch options: a run-level `--limit`/`--sample-id` slices the added task's dataset and a run-level `--epochs` overrides the task's) — without adding anything. The command reports "applied" when the add is accepted and scheduled, not when it has run: track progress with `inspect ctl task list`, keyed by the returned task ids. The target is a *process*, not a task — the sole running (or sole addable) process is the default and `--pid` disambiguates — and the command is safe against duplicate delivery: each invocation carries an idempotency key, so a transport-level retry can't add the task twice (invoking the command twice on purpose adds it twice, by design).
