...
- Only the captured paths are written. A snapshot node outside them is refused, as is any device or fifo node, and any file or directory carrying a setuid, setgid or sticky bit. An agent that builds a setuid binary or a `chmod g+s` directory under a captured path makes that sample unresumable; keep such files outside `sandbox_paths`.
