...
The same buffer backs the control channel's `inspect ctl task add`, which submits a task *spec* from outside the process — see [Adding Tasks](control-channel.qmd#adding-tasks).

## Concurrency

Any run executing tasks concurrently (`max_tasks` above 1) is live: a task added mid-run — by a `TaskSource`, `enqueue_task()`, or `inspect ctl task add` — starts as soon as there is free capacity, rather than waiting for a batch boundary. (With `max_tasks` of 1, additions run as a follow-up batch after the in-flight batch completes, preserving task ordering.)

A blocked `next_tasks()` does not hold additions back: a task added while the poll is blocked starts anyway (at `max_tasks` of 1, as the next follow-up batch), with the poll continuing underneath. This means `next_tasks()` can resolve concurrently with running tasks, rather than only while the run is otherwise idle.
