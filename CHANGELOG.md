...
- Eval Set: A retry attempt's log now starts out holding every sample record from the prior attempt's log, so a retry that itself errors, is cancelled, or is interrupted no longer causes the next attempt to re-run (or, with `retry_cleanup`, permanently lose) samples that an earlier attempt completed.
- Eval Set: A retry attempt now copies the prior attempt's log before its first sample runs (one whole-file read rather than a per-sample read of the prior log), and a retry that runs with `log_images=False` keeps the images already recorded in the prior attempt's samples.
