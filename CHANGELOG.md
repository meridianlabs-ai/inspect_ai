...
- Control Channel: the control server now reports a control-API version on `GET /tasks`, and `inspect ctl config` rejects knobs the target process's (older) inspect version doesn't support before applying any change, instead of letting them silently no-op.
