...
- Control Channel: A `--ctl-server=keep` park now auto-releases after 24 hours idle so a forgotten release no longer leaks the process (tune with `keep:<idle>` / `keep:forever`, or live via `inspect ctl config --park-timeout`).
