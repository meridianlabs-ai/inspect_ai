...
`POST /tasks` (CLI: `inspect ctl task add SPEC [...]`) submits a **task spec** that runs in the target process under the same `run_id`, appearing as a new sibling eval in `task list` / `sample list` / `sample events`. [`task-add.md`](task-add.md) owns the full design (accept/reject semantics, the park drain loop, idempotency, identity) and supersedes the sketch below where they differ — notably the response returns the stable `task_id` rather than an `eval_id`, the inject path does *not* "mint identity + logger" in the accept path (logger init deliberately stays inside `prepare_options`, off the request path — the reason `task_id` is the returned handle), addability gates on the live keep intent rather than the launch flag alone, and the pre-started-workers bullet is obsolete (the current dispatcher's wake/feed loop dispatches injected work on free capacity without a worker pool).
