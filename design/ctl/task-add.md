...
3. **Feed wiring for every `parallel > 1` run** (dormant unless fed), replacing the `task_source`-only gate in `eval_async` — restarted park sessions included; `parallel == 1` keeps `run_batches` follow-up-batch semantics. Per-batch containment of `prepare_options` failures on the feed path (see Failure modes).
...
5. **Accept-path plumbing:** resolve → (optionally) dedup by `request_id` → enqueue pre-resolved → fire `notify_park_change` → return `{task_id, ...}`. Resolution and enqueue are synchronous on the eval loop; the expensive preparation (logger init, sandbox startup) stays where it is today, inside `eval_run`'s `prepare_options`, off the request path.
