...
- **Response** rides the uniform mutation envelope `{target, applied, dry_run, detail}`. `detail` carries the resolved identity available at accept time: task name, sample count, epochs, and a **`tasks` list of `{task_id, model}` rows — one per resolved task**, since the default (no `--model`) resolves against every launch model and mints one `ResolvedTask` per model; a single-model run or an explicit `--model` yields a one-row list, and the shape doesn't fork on that. See "Identity" below for why `task_id` (not `eval_id`) is the returned handle.
...
- **No `CONTROL_API_VERSION` gate — but the skew signal is 405, not 404.** Unlike `sample requeue` (a genuinely new *path*, where an older server answers the stock 404), `/tasks` already exists on every server as the task listing — so on an older server `POST /tasks` *path-matches* and FastAPI answers the stock `{"detail": "Method Not Allowed"}` **405**. That is just as unambiguous pre-task-add (once the route ships, the POST matches it), so the CLI keys route-missing detection on 405 for this endpoint: a method-not-allowed sibling of `_request_json`'s existing 404 hook (`not_found_missing_route` fires only on 404 today), reporting "older inspect — restart the eval" definitively.
...
- `run_session` is the session launcher: a fresh `eval_run` with the injection feed wired at `parallel > 1` (so an add landing *while a restarted session runs* injects live, same as the original body — restarted sessions follow the same feed-wiring rule, not `run_batches`' batch boundaries) and `run_batches`' sequence-grouped batching at `parallel == 1`. It is deliberately **source-free**: it never consults `task_source.next_tasks()` — a TaskSource-driven run only reaches the park after its source returned `None`, and re-polling a source that may block indefinitely would hang the park instead of re-parking. Each session opens its own task display, prepares options (logger init, incremental sandbox startup), and dispatches. Same `run_id` (the enqueuer is run-scoped and validates it), so the new task registers as a sibling of the originals; the `EvalState` registry is cleared only at the outermost run boundary *after* the park (`reset_run_registries` in `eval_async`'s `finally`), so prior tasks stay visible in `task list` alongside the added one throughout.
...
3. **Feed wiring for every `parallel > 1` run** (dormant unless fed), replacing the `task_source`-only gate in `eval_async` — restarted park sessions included; `parallel == 1` keeps `run_batches` follow-up-batch semantics. This changes the documented scheduling of `enqueue_task` additions on non-source `parallel > 1` runs (see Inject), so the same change updates the `enqueue_task` docstring and `docs/task-source.qmd`, forwards `debug_errors` uniformly, and adds per-batch containment of `prepare_options` failures on the feed path (see Failure modes).
4. **The park loop** sketched above: drain-adds arm via the source-free `run_session`, widened wake condition (with the under-condition re-check), widened entry gate (`intent or pending`), log accumulation across sessions.
...
- `_eval/eval.py` — register the capability; `(task_args, model)` parameters on `_resolve_enqueued_tasks`; wire the injection feed for all `parallel > 1` runs; the park drain loop (source-free `run_session`) and widened entry gate; log accumulation.
- `_eval/task/enqueue.py` — a pre-resolved enqueue entry point; possibly a small hook so the capability can observe "run tearing down".
- `_eval/run.py` — per-batch containment of feed-path `prepare_options` failures.
