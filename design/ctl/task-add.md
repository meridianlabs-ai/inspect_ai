...
5. **Accept-path plumbing:** resolve → (optionally) dedup by `request_id` → enqueue pre-resolved (firing `on_enqueue`, the dispatch waker) → fire `notify_park_change` → return `{task_id, ...}`. Resolution and enqueue are synchronous on the eval loop; the expensive preparation (logger init, sandbox startup) stays where it is today, inside `eval_run`'s `prepare_options`, off the request path.
